<script lang="ts">
  import { chatStore, type Chat } from '$lib/stores/chatStore';
...
<aside class="flex flex-col h-full {$isCollapsed ? 'w-[60px]' : 'w-[220px]'} bg-[#1a1a1a] border-r border-gray-800 transition-all duration-300">
  ...
  <div class="flex-1 overflow-y-auto">
    <div class="px-2 space-y-1">
      {#each chats as chat}
        <button
          class="w-full text-left px-3 py-2 rounded-md flex items-center space-x-2 {activeChat === chat.id ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}"
